...
namespace _2021_dotnet_e_02.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
        ...
        private const int USER_LOGIN_MAX_ATTEMPTS = 5;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (done)
                {
                    if (javaUser.Status == UserStatus.BLOCKED)
                    {
                        javaUser.IncreaseFailedLoginAttempts();
                        javaUser.AddLoginAttempt(new LoginAttempt() { DateAndTime = DateTime.Now, LoginStatus = LoginStatus.FAILED, UserModel = javaUser, Username = javaUser.UserName });
                        _userRepository.Update(javaUser);
                        ...
                        if (result.Succeeded)
                        {
                            ...
                            javaUser.ResetFailedLoginAttempts();
                            javaUser.AddLoginAttempt(new LoginAttempt() { DateAndTime = DateTime.Now, LoginStatus = LoginStatus.SUCCESS, UserModel = javaUser, Username = javaUser.UserName });
                            _userRepository.Update(javaUser);
                            _userRepository.SaveChanges();
                        ...
                        /*else*/ if(!result.Succeeded)
                        {
                            ...
                            if (javaUser.FailedLoginAttempts >= USER_LOGIN_MAX_ATTEMPTS)
                            {
                                javaUser.BlockUser();
